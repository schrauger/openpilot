...
class CarState(object):
  ...
  def update(self, cp):
    ...
    if self.CP.carFingerprint in TSS2_CAR:
      ...
      self.zss = cp.vl["SECONDARY_STEER_ANGLE"]['ZORRO_STEER'] - self.angle_offset
      self.shitty_angle = cp.vl["STEER_ANGLE_SENSOR"]['STEER_ANGLE'] + cp.vl["STEER_ANGLE_SENSOR"]['STEER_FRACTION']
      if abs(self.shitty_angle) > 1e-3 and abs(self.angle_steers) > 1e-3 and not self.init_angle_offset:
        self.init_angle_offset = True
        self.angle_offset = self.angle_steers - self.shitty_angle
    else:
